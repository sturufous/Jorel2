...
public class NewsItemsDao extends Jorel2Root implements java.io.Serializable {
	 ...
	 * @return The number of NEWS_ITMES records generated by this publisher
	 */
	public static Long getNewsItemCountByWebPathAndSource(String webPath, String source, Session session) {
		
		long result = 0;
		
		String sqlStmt = "select count(*) as count from NewsItemsDao where webPath = :webPath and (source = :source or source = 'social_media_junk')";
		
		Query<Long> query = session.createQuery(sqlStmt, Long.class);
		query.setParameter("webPath", webPath);
		query.setParameter("source", source);
        List<Long> results = query.getResultList();
	 ...
	 * @return A list containing all the records that meet the expiry criteria.
	 */
	public static List<NewsItemsDao> getExpiredItems(String source, BigDecimal days, Session session) {
		
		String sqlStmt = "from NewsItemsDao where source = :source and itemDate < (sysdate - :days) and expireRule = 0 and archived = 0";
	 ...
	 */
	public static String getAlertItemRsns(Session session) {
        ...
        if (results.size() > 0) {
        	rsnList = results.toString().replace("[", "");
        	rsnList = rsnList.replace("]", "");
        }
	 ...
	 * @param session The current Hibernate persistence context.
	 */
	public static void deleteRecord(NewsItemsDao item, Session session) {
